...
CREATE CERTIFICATE [CodeSigningCertificate]	ENCRYPTION BY PASSWORD = 'Query@Stack' WITH EXPIRY_DATE = '2099-01-01' ,SUBJECT = 'DBA Code Signing Cert'
GO

BACKUP CERTIFICATE [CodeSigningCertificate] TO FILE = 'C:\temp\CodeSigningCertificate.cer'
	WITH PRIVATE KEY (FILE = 'C:\temp\CodeSigningCertificate_WithKey.pvk', ENCRYPTION BY PASSWORD = 'Query@Stack', DECRYPTION BY PASSWORD = 'Query@Stack' );
GO
...
CREATE CERTIFICATE [CodeSigningCertificate] FROM FILE = 'C:\temp\CodeSigningCertificate.cer'
	WITH PRIVATE KEY (FILE = 'C:\temp\CodeSigningCertificate_WithKey.pvk',
					  ENCRYPTION BY PASSWORD = 'Query@Stack',
					  DECRYPTION BY PASSWORD = 'Query@Stack'
					  );
GO
...
ADD SIGNATURE TO [dbo].[sp_Kill] BY CERTIFICATE [CodeSigningCertificate] WITH PASSWORD = 'Query@Stack' -- 'Work@Y0urBest'
GO

GRANT EXECUTE ON OBJECT::[dbo].[sp_Kill] TO [public]
GO

ADD SIGNATURE TO [dbo].[sp_WhoIsActive] BY CERTIFICATE [CodeSigningCertificate] WITH PASSWORD = 'Query@Stack' -- 'Work@Y0urBest'
GO

GRANT EXECUTE ON OBJECT::[dbo].[sp_WhoIsActive] TO [public]
GO

ADD SIGNATURE TO [dbo].[sp_HealthCheck] BY CERTIFICATE [CodeSigningCertificate] WITH PASSWORD = 'Query@Stack' -- 'Work@Y0urBest'
GO
...
ADD SIGNATURE TO [dbo].[usp_WhoIsActive_Blocking] BY CERTIFICATE [CodeSigningCertificate] WITH PASSWORD = 'Query@Stack' -- 'Work@Y0urBest'
GO
