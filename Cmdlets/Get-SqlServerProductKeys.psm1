Function Get-SqlServerProductKeys {
	<# 
	 .SYNOPSIS 
        ...
        try
        {
		    ...
		    foreach ($servername in $servers) {
			    ...
			    foreach ($instance in $instances.GetValueNames()) {
				    if ($instance -eq "MSSQLSERVER") { $sqlserver = if($servername  -match "^(?'ServerName'[0-9A-Za-z_-]+)\.*?.*"){$Matches['ServerName']}else{$null} } else { $sqlserver = "$servername\$instance" }
                    ...
                    $pServerName = if($servername  -match "^(?'ServerName'[0-9A-Za-z_-]+)\.*?.*"){$Matches['ServerName']}else{$null}
		    ...
		    $objectCollection | Write-Output #| Select "SQL Instance", "SQL Version", "SQL Edition", "Product Key" 
	    }
        catch
        {
            $returnMessage = $null;
