Function Add-ServerInfo
{
    ...
    if ([String]::IsNullOrEmpty($ServerName) -or (Test-Connection -ComputerName $ServerName -Count 1 -Quiet) -eq $false)
    {
        ...
        if($LogErrorToInventoryTable) {
            Add-CollectionError -ComputerName $ServerName -Cmdlet 'Add-ServerInfo' -CommandText "Add-ServerInfo -ServerName '$ServerName'" `
    ...
    {
        $ServerName = (Get-FullQualifiedDomainName -ComputerName $ServerName);
        Write-Verbose "`$FQDN = $ServerName";
        
        Write-Verbose "  Checking if '$ServerName' is already present in Inventory";
        $sqlQuery = @"
select 1 as IsPresent from [$InventoryDatabase].[dbo].[Server] where FQDN = '$ServerName'
"@;
        #Write-Host "$sqlQuery";
        $Tables = $null;
        try 
    ...
    if ($AddSwitch) 
    {
        ...
        Write-Verbose "  Calling Get-ServerInfo -ServerName $ServerName";

        $serverInfo = Get-ServerInfo -ServerName $ServerName | Select-Object @{l='ServerName';e={if($_.ServerName  -match "^(?'ServerName'[0-9A-Za-z_-]+)\.*?.*"){$Matches['ServerName']}else{$null}}}, @{l='ApplicationId';e={$ApplicationID}}, @{l='EnvironmentType';e={$EnvironmentType}}, FQDN, IPAddress, Domain, 
        ...
        {
            $MessageText = "Get-WmiObject : Access is denied. Failed in execution of Get-ServerInfo -ServerName $ServerName";
            if($LogErrorToInventoryTable) {
                Add-CollectionError -ComputerName $ServerName -Cmdlet 'Add-ServerInfo' -CommandText "Add-ServerInfo -ComputerName '$ServerName'" `
        ...
        foreach ($i in $serverInfo)
        {
            ...
            try 
            {
                if ($AddSwitch) 
                {
                    ...
                    $i | Write-DbaDataTable -SqlInstance $InventoryInstance -Database $InventoryDatabase -Schema 'Staging' -Table 'ServerInfo' -AutoCreateTable;
                    #$i | Write-DbaDataTable -SqlInstance $InventoryInstance -Database $InventoryDatabase -Schema 'Staging' -Table 'ServerInfo_new' -AutoCreateTable;
                    <#
    ...
    Add-ServerInfo -ServerName $ServerName -Verbose
    $returnMessage
"@ | Out-Host;
                #>
                Write-Host "Error occurred in while trying to get Add-ServerInfo for server [$ServerName].";
                return
            }
        }
...
"@;
    ...
    {
        Add-SqlInstanceInfo -ServerName $ServerName -CallTSQLProcedure Yes;
    }
<#
    .SYNOPSIS 
